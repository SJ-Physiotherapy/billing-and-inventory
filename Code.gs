...
const THEME_SETTING_DEFAULTS = [
  ...
  // --- Bill/PDF design - one shared design used everywhere a bill is shown:
  //     on-screen preview, Print, Save-as-PDF, and the emailed PDF/email
  //     body. Font family applies to the whole bill (name, info, invoice
  //     number, table, totals) - a bill mixing multiple fonts looks
  //     unprofessional, so this is intentionally one shared choice rather
  //     than a separate one per element. ---
  ['ThemeBillFontFamily', "Georgia, 'Times New Roman', Times, serif"],
  ['ThemeBillDiscountColor', '#B23A2E'],
  ['ThemeBillLogoWidth', '96'],
  ['ThemeBillLogoHeight', '58'],

  // --- Buttons - normal AND hover colors, for both the solid/gradient
  //     "primary" buttons (Save & Generate Bill, etc.) and the bordered
  //     "secondary/outline" buttons (Logout, Refresh, Check Data Status,
  //     Reset Numbering, and every other button that shares this same
  //     style). ---
...
function migrateSchema_() {
  ...
  if (custSh && custSh.getLastColumn() > 0) {
    ...
    if (String(headerCell.getValue()).trim() === 'Name') headerCell.setValue('Patient Name');

    if (custSh.getLastRow() > 1) {
      const custData = custSh.getRange(2, 1, custSh.getLastRow() - 1, 2).getValues(); // [PatientID, Name]
      const blankRows = [];
...
function doGet(e) {
  try {
    ...
    // session token issued at login - except getSettings, which the login
    // screen itself calls before anyone has a session (to theme the login
    // gradient/card from Admin Settings). It only returns theme/company
    // display info, never bills, customers, or anything sensitive.
...
// app knows about (the active one, plus every archived one) in one call:
// name/link, Active vs Archived, capacity %, and Bills/Customers/Products
// row counts. Archived databases are frozen (read-only) so they don't get
// "Next ID" predictions - those only make sense for the one database new
// bills actually get written into.
function apiGetDataDiagnostics(p) {
  ...
  function dbInfo_(ssRef) {
    const billsSh = ssRef.getSheetByName(SHEET.BILLS);
    const custSh = ssRef.getSheetByName(SHEET.CUSTOMERS);
    const prodSh = ssRef.getSheetByName(SHEET.PRODUCTS);
    let totalCells = 0;
    ssRef.getSheets().forEach(function (sh) { totalCells += sh.getMaxRows() * sh.getMaxColumns(); });
    return {
      name: ssRef.getName(),
      url: ssRef.getUrl(),
      ...
      billsSh: billsSh, custSh: custSh, prodSh: prodSh
    };
  }
  ...
  const active = {
    ...
    nextCustomerId: nextSequentialId_('IDCTR_PAT', 'PAT', 4, activeInfo.custSh, 0, false),
    nextProductId: nextSequentialId_('IDCTR_PROD', 'PROD', 4, activeInfo.prodSh, 0, false)
  };

  const archives = getArchiveRegistry_().map(function (entry) {
    try {
...
function apiGetSettingsPublic() {
  ...
  return {
    ...
    settings: Object.assign({
      ...
      // Only ever used in the emailed invoice's footer social icons (see
      // buildBrandedEmailShell_) - fetched from these Shop Details fields
      // ONLY, nothing hardcoded/defaulted here. Leave a field blank in
      // Shop Details to simply not show that icon in the email.
...
// ---------------------------------------------------------------------------
// 8b. EMAIL BILL - a fully-branded email (same header/footer look on every
//     email this shop sends), with the bill itself shown as a clean summary
//     table right in the email body PLUS the full invoice attached as a
//     PDF - the same "here's what happened, and here's your formal
//     document too" pattern big companies (Amazon, etc.) use. The exact
//     same buildBrandedEmailShell_/buildEmailBillBodyHtml_ functions power
//     both the real send below AND apiGetEmailPreview (what the Share Bill
//     modal shows before anyone clicks Send) - so the preview is never a
//     lie about what the customer will actually receive. The very first
//     time this runs after a fresh deployment, Google will prompt to
//     authorize an extra "send email" permission - that's expected, just
//     accept it once.
// ---------------------------------------------------------------------------
function apiEmailBillPdf(p) {
  ...
  try {
    const pdfHtml = buildBillHtmlForPdf_(bill, s, totals);
    const pdfBlob = HtmlService.createHtmlOutput(pdfHtml).getAs('application/pdf').setName('Invoice-' + bill.billId + '.pdf');
    const emailHtml = buildBrandedEmailShell_(s, buildEmailBillBodyHtml_(bill, s, totals));
    MailApp.sendEmail({
      to: email,
      name: s.CompanyName || 'Invoice',
      subject: 'Invoice ' + bill.billId + ' - ' + (s.CompanyName || 'Invoice'),
      htmlBody: emailHtml,
      attachments: [pdfBlob]
    });
    return { ok: true };
...
// Read-only - builds and returns the exact same HTML apiEmailBillPdf would
// send, without sending anything. Powers the "Preview Email" step in the
// Share Bill modal.
function apiGetEmailPreview(p) {
  ...
  const s = apiGetSettingsPublic().settings;
  const totals = computeBillTotals_(bill.items, bill.discountPercent, bill.taxOverride, s);
  return { ok: true, html: buildBrandedEmailShell_(s, buildEmailBillBodyHtml_(bill, s, totals)) };
}

// The email BODY content - greeting + a clean bill summary table (product /
// qty / total, discounts, grand total) - built with plain HTML tables and
// inline styles throughout (no external stylesheet, no flexbox/grid - email
// clients like Outlook only reliably render table-based layouts). Shares
// its number-crunching (totals) with the on-screen preview, print, and PDF -
// only the markup differs, never the numbers.
function buildEmailBillBodyHtml_(bill, s, totals) {
  const tax = totals.tax;
  // Every piece of text in the email body (including the bill table) is
  // plain black - simpler and easier to read at a glance than the app's
  // usual heading/muted/brand color mix, per how this email is meant to
  // read. Only the discount figures keep their own color, since that's
  // meaningful information (a discount), not decoration.
  const ink = '#000000';
  const border = s.ThemeBorderColor || '#E0E0E0';
  const discountColor = s.ThemeBillDiscountColor || '#B23A2E';
  // Georgia, fixed - this email's typography is intentionally independent
  // of the Bill/PDF Design font choice (Admin Settings -> Theme), which
  // only controls the on-screen preview/print/PDF, not this email.
  const font = "font-family:Georgia,'Times New Roman',Times,serif;";
  const hasAnyItemDiscount = totals.itemDiscountTotal > 0.004;

  // Discount gets its OWN column (matching the PDF/print table exactly) -
  // appending "-4%" straight onto the product name read as part of the
  // name, not a discount, to anyone glancing at the email.

  const rows = totals.items.map(function (i) {
    const discCell = i.discountPercent > 0 ? '-' + i.discountPercent + '%' : '&ndash;';
    return '<tr>' +
      '<td style="padding:12px 10px;border-bottom:1px solid ' + border + ';' + font + 'font-size:14px;line-height:22px;color:' + ink + ';">' + escHtml_(i.name) + '</td>' +
      '<td align="center" style="padding:12px 10px;border-bottom:1px solid ' + border + ';' + font + 'font-size:14px;line-height:22px;color:' + ink + ';">' + i.qty + '</td>' +
      '<td align="center" style="padding:12px 10px;border-bottom:1px solid ' + border + ';' + font + 'font-size:14px;line-height:22px;color:' + ink + ';">' + discCell + '</td>' +
      '<td align="right" style="padding:12px 10px;border-bottom:1px solid ' + border + ';' + font + 'font-size:14px;line-height:22px;color:' + ink + ';font-weight:700;">\u20B9' + Number(i.lineTotal).toFixed(2) + '</td>' +
      '</tr>';
  }).join('');

  let totalsRows = '';
  const totalRow_ = function (label, value, isGrand) {
    return '<tr>' +
      '<td colspan="3" align="right" style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;color:' + ink + ';font-weight:' + (isGrand ? '800' : '600') + ';' + (isGrand ? 'border-top:2px solid ' + ink + ';' : '') + '">' + label + '</td>' +
      '<td align="right" style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;color:' + ink + ';font-weight:800;' + (isGrand ? 'border-top:2px solid ' + ink + ';' : '') + '">' + value + '</td>' +
      '</tr>';
  };
  totalsRows += totalRow_('Item Subtotal', '\u20B9' + totals.grossSubtotal.toFixed(2));
  if (hasAnyItemDiscount) totalsRows += totalRow_('Item Discounts', '-\u20B9' + totals.itemDiscountTotal.toFixed(2));
  if (totals.totalDiscPct > 0) totalsRows += totalRow_('Additional Discount (' + totals.totalDiscPct + '%)', '-\u20B9' + totals.totalDiscountAmount.toFixed(2));
  if (tax.showTaxSection) totalsRows += totalRow_('Tax', '\u20B9' + tax.taxTotal.toFixed(2));
  totalsRows += totalRow_('Grand Total', '\u20B9' + tax.grandTotal.toFixed(2), true);

  return '' +
    '<tr><td align="left" style="' + font + 'font-size:14px;line-height:22px;color:' + ink + ';padding-bottom:22px;">' +
    '<b>Hi ' + escHtml_(bill.customerName || 'there') + ',</b><br><br>' +
    'Thank you for choosing ' + escHtml_(s.CompanyName || 'us') + '. Here\u2019s a summary of Invoice <b>' + escHtml_(String(bill.billId)) + '</b>, dated ' + escHtml_(formatDate_(bill.date)) + '. The full invoice is also attached to this email as a PDF.' +
    '</td></tr>' +
    ...
    '<tr>' +
    '<td style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;text-transform:uppercase;letter-spacing:.3px;color:' + ink + ';border-bottom:1.5px solid ' + ink + ';">Service / Treatment</td>' +
    '<td align="center" style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;text-transform:uppercase;letter-spacing:.3px;color:' + ink + ';border-bottom:1.5px solid ' + ink + ';">Qty</td>' +
    '<td align="center" style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;text-transform:uppercase;letter-spacing:.3px;color:' + ink + ';border-bottom:1.5px solid ' + ink + ';">Discount</td>' +
    '<td align="right" style="padding:12px 10px;' + font + 'font-size:14px;line-height:22px;text-transform:uppercase;letter-spacing:.3px;color:' + ink + ';border-bottom:1.5px solid ' + ink + ';">Total</td>' +
    '</tr>' +
    rows +
    totalsRows +
    '</table>' +
    ...
    'Questions about this invoice? Just reply to this email' + (s.Phone ? ' or call us at <b>' + escHtml_(s.Phone) + '</b>' : '') + '.<br><br>' +
    'Warm regards,<br><b>Team ' + escHtml_(s.CompanyName || '') + '</b>' +
    '</td></tr>';
}

// The reusable branded shell every invoice email uses - header (logo on a
// fixed brand gradient) + the caller's body content + footer (address +
// social icons on the same gradient), copied element-for-element from the
// reference email.html template: same two gradient colors, same 7 social
// icons/links in the same order, no copyright line. Table-based markup and
// inline styles throughout, plus a <=600px media query, so this renders
// correctly and responsively in Gmail, Outlook, and mobile mail apps alike.
function buildBrandedEmailShell_(s, bodyRowsHtml) {
  // Georgia, fixed - independent of the Bill/PDF Design font setting (which
  // only governs the on-screen preview/print/PDF), so this email's
  // typography never changes just because someone picks a different font
  // for the printed invoice.
  const font = "Georgia, 'Times New Roman', Times, serif";
  // Fixed - this is the shop's one email brand identity (from the reference
  // email.html), independent of the in-app Theme colors, so the email
  // never drifts from that design no matter what color the app UI is using.
  const gradFrom = '#2778b7';
  const gradTo = '#a8d339';
  const gradText = '#FFFFFF';
  // Fixed rectangular wordmark logo, specifically for the email header -
  // deliberately NOT the app's LogoURL/PrintLogoURL (those are square,
  // sized for the sidebar/login and the bill respectively; a square logo
  // stretched into a wide header band looks wrong, so the email gets its
  // own logo suited to that shape).
  const logo = 'https://sjphysiotherapy.in/LOGO/SJ-PHYSIOTHERAPY-EMAIL-LOGO.png';
  const companyName = escHtml_(s.CompanyName || '');

  // Same 7 icons, same order, same link pattern as the reference
  // email.html - phone/mail come from Settings (Phone/CompanyEmail); the
  // rest come from the Social Links fields in Admin Settings -> Shop
  // Details, so every link here is fetched from Settings, nowhere else.
  ...
  return '<!DOCTYPE html><html xmlns="http://www.w3.org/1999/xhtml"><head><meta charset="UTF-8">' +
    ...
    // Header - just the rectangular logo (it already contains the company
    // name as a wordmark), on the fixed brand gradient.
    ...
    // Body - generous padding/line-height throughout, not congested.
    ...
    // Footer - address + social icons only, no copyright line.
    '<tr><td style="padding:0;">' +
    '<div style="background-color:' + gradFrom + ';background-image:linear-gradient(135deg, ' + gradFrom + ' 0%, ' + gradTo + ' 100%);">' +
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>' +
    ...
    '<b>' + companyName + '</b>' + (s.Address ? ' &nbsp;' + escHtml_(s.Address) : '') +
    '</td></tr></table>' +
    socialHtml +
    '</td></tr></table></div></td></tr>' +
...
function buildBillHtmlForPdf_(bill, s, totals) {
  ...
  const tableHead = '<tr><th>Service / Treatment</th><th class="num">Price</th><th class="num">Qty</th>' +
  ...
  const leftMeta = [
    ...
    'Patient: <b>' + escHtml_(bill.customerName || '') + '</b>',
    'Patient ID: <b>' + escHtml_(bill.customerId || '') + '</b>',
  ...
  const themeDiscount = s.ThemeBillDiscountColor || '#B23A2E';
  const billFont = s.ThemeBillFontFamily || "Georgia, 'Times New Roman', Times, serif";
  const logoW = Math.max(20, Number(s.ThemeBillLogoWidth) || 96);
  const logoH = Math.max(20, Number(s.ThemeBillLogoHeight) || 58);

  // Bill typography - company name and company info (address/phone/etc.)
  // each get their own independent bold/italic/underline switches, one
  // shared font family, and the header can lay the logo out beside the
  // info or stacked above it. These are the EXACT same Theme settings that
  // drive the on-screen preview in app.js (buildCompanyHeadHtml) and the
  // print stylesheet (style.css) - this function is a deliberate one-to-one
  // mirror of that same markup/CSS, so the emailed PDF is never a
  // different-looking document from what was previewed, printed, or saved
  ...
  const headHtml = headerLayout === 'logo-top'
    ? '<div class="bill-head bill-head-stacked">' +
      ...
      (logo ? '<img class="bill-logo" src="' + logo + '">' : '<span></span>') +
      '<div class="bill-tag"><div class="bill-tag-label">Invoice</div><div class="big">' + escHtml_(String(bill.billId)) + '</div></div></div>' +
  ...
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=750"><style>' +
    // html/body get an EXPLICIT fixed pixel width, not a percentage or an
    // auto-margined column - Apps Script's HTML-to-PDF conversion does not
    // reliably expose a predictable "page width" for percentages to resolve
    // against (that ambiguity is exactly what caused this to alternate
    // between "huge blank margins" and "content cropped off the right edge"
    // in earlier attempts). Declaring the width outright removes the
    // guesswork: .bill-page below is 100% of THIS explicit 750px, so it
    // always renders at the same true size, filling the page with sensible
    // margins and never overflowing off the edge.
    'html,body{width:750px;margin:0;padding:0;}' +
    'body{font-family:' + billFont + ';color:' + themeHeading + ';background:#FFFFFE;}' +
    '.bill-page{width:100%;box-sizing:border-box;padding:24px 26px;font-size:13px;}' +
    ...
    '.bill-logo{width:' + logoW + 'px;height:' + logoH + 'px;max-width:' + logoW + 'px;max-height:' + logoH + 'px;border-radius:4px;object-fit:contain;background:#fff;flex-shrink:0;}' +
    ...
    '.co-meta{font-size:11px;font-weight:' + infoWeight + ';font-style:' + infoStyle + ';text-decoration:' + infoDecoration + ';color:' + themeBillHeader + ';line-height:1.55;margin:0;}' +
    ...
    '.bill-table td.disc{color:' + themeDiscount + ';font-size:11.5px;}' +
    ...
    '.bill-totals .row.disc{color:' + themeDiscount + ';font-size:12px;}' +
    ...
    '</style></head><body><div class="bill-page">' +
    headHtml +
    ...
    'Thank you for choosing ' + escHtml_(s.CompanyName || 'us') + '!' + bankHtml + '</div>' +
    signHtml +
    '</div>' +
    '</div></body></html>';
}
...
// "Qty" and "Net LineTotal" are unique per row (safe to SUM directly in
...
//
// --- Discount breakdown, one column per stage of the calculation -----
// Every bill can carry TWO independent discounts (see computeBillTotals_):
// an ITEM discount (set per product line, "Item Discount %") applied
// first, then a BILL-level "additional discount" (set once for the whole
// bill, "Bill Discount %") applied to whatever's left after that. Both are
// broken out here as their own %-and-₹ column pair so neither discount is
// ever hidden inside a single lump number:
//   Gross Amount  -->  - Item Discount Amount  -->  - Bill Discount Amount  -->  Net LineTotal
// "Gross Amount", "Item Discount Amount" and "Bill Discount Amount" are ALL
// unique per row (safe to SUM directly, unlike the two "Bill Total ..."
// columns above) - "Bill Discount Amount" is this line's own proportional
// share of that bill's additional discount, not the bill's discount
// repeated on every line, so summing it across a bill's rows correctly
// reconstructs that bill's total additional-discount amount with no
// double-counting. The two "%" columns are rates, not money - they repeat
// per bill by design and are not meant to be summed.
const UNIVERSAL_REPORT_HEADERS = [
  'CustomerID', 'Patient Name', 'Phone', 'Email', 'BillID', 'BillerID', 'Biller Name',
  'Invoice Date', 'Invoice Time', 'CreatedAt', 'Address', 'DeliveryAddress',
  'ProductName', 'UnitPrice', 'Qty', 'Gross Amount',
  'Item Discount %', 'Item Discount Amount',
  'Bill Discount %', 'Bill Discount Amount',
  'Net LineTotal', 'PaymentMethod',
...
function apiGetUniversalReport() {
  ...
  allDbs_().forEach(db => {
    ...
    for (let i = 1; i < itemsData.length; i++) {
      ...
      // --- Discount breakdown for THIS product line, in the same order
      // discounts are actually applied (see computeBillTotals_): the
      // item's own % first, then the bill's "additional discount" % on
      // whatever's left. "LineTotal" (item[4]) is stored ALREADY net of
      // BOTH discounts, so billDiscountAmount is derived directly from it
      // rather than recomputed independently - that keeps
      // grossAmount = itemDiscountAmount + billDiscountAmount + lineTotal
      // exactly, with no floating-point drift between this report and
      // what was actually billed. ---
      const unitPrice = Number(item[2]) || 0;
      const qty = Number(item[3]) || 0;
      const lineTotal = Number(item[4]) || 0; // net, after BOTH discounts
      const itemDiscountPercent = Number(item[5]) || 0;
      const billDiscountPercent = Number(bill[22]) || 0;
      const grossAmount = unitPrice * qty;
      const itemDiscountAmount = grossAmount * itemDiscountPercent / 100;
      const afterItemDiscount = grossAmount - itemDiscountAmount;
      const billDiscountAmount = Math.max(0, afterItemDiscount - lineTotal);
...
function buildUniversalReportSheet_() {
  ...
  let totalGrossAmount = 0, totalItemDiscount = 0, totalBillDiscount = 0, totalLineRevenue = 0, totalQtySold = 0;
  for (let i = 1; i < itemsData.length; i++) {
    const item = itemsData[i];
    if (!item[0]) continue;
    const bill = billMap[String(item[0])];
    ...
    // Same discount breakdown, and for the same reason, as in
    // apiGetUniversalReport() above - see the comment there.
    const unitPrice = Number(item[2]) || 0;
    const qty = Number(item[3]) || 0;
    const lineTotal = Number(item[4]) || 0; // net, after BOTH discounts
    const itemDiscountPercent = Number(item[5]) || 0;
    ...
    totalBillDiscount += billDiscountAmount;
    ...
    dataRows.push([
      ...
      bill[6], bill[7],
      item[1], unitPrice, qty, grossAmount,
      itemDiscountPercent, itemDiscountAmount,
  ...
  const totalDiscountGiven = totalItemDiscount + totalBillDiscount;

  // --- Rows 1-3: a plain-language summary block with the CORRECT,
  //     already-deduplicated totals, so nobody has to manually select a
  //     column and sum it (and risk summing the wrong one). Gross, discount
  //     and net are all shown together so it's immediately clear how much
  //     was actually earned AFTER both the item-level and bill-level
  //     discounts, not just the headline sales figure. ---
  sh.getRange(1, 1, 1, numCols).merge();
  sh.getRange(1, 1)
    .setValue('SALES SUMMARY - ' + seenBillIds.size + ' bill(s)  |  Gross (Before Discount): ₹' +
      totalGrossAmount.toLocaleString('en-IN') + '  |  Total Discount Given: ₹' + totalDiscountGiven.toLocaleString('en-IN') +
      ' (Item: ₹' + totalItemDiscount.toLocaleString('en-IN') + ' + Bill: ₹' + totalBillDiscount.toLocaleString('en-IN') + ')' +
      '  |  Net Sales (Actually Earned): ₹' + totalLineRevenue.toLocaleString('en-IN') + '  |  Total Qty Sold: ' + totalQtySold)
  ...
  sh.getRange(2, 1)
    .setValue('⚠ "Bill Total Qty" and "Bill Total Amount" repeat once per product line within the same bill - ' +
      'do NOT sum those two columns directly, they will over-count. "Qty", "Gross Amount", "Item Discount Amount", ' +
      '"Bill Discount Amount" and "Net LineTotal" are all unique per row and safe to sum (the two "%" discount ' +
      'columns are rates, not money, and are not meant to be summed). Filter "First Line Of Bill" = TRUE first if ' +
      'you need to sum the two bill-level columns instead.')
  ...
  // Looked up by header NAME rather than a hardcoded column number, so this
  // still points at the right cells if columns are ever reordered again.
  const billTotalQtyCol = UNIVERSAL_REPORT_HEADERS.indexOf('Bill Total Qty') + 1;
  const billTotalAmountCol = UNIVERSAL_REPORT_HEADERS.indexOf('Bill Total Amount') + 1;
  const noteText = 'Repeats once per product line on the same bill - do not SUM this column directly. See the summary at the top of this sheet, or filter "First Line Of Bill" = TRUE first.';
  if (billTotalQtyCol > 0) sh.getRange(HEADER_ROW, billTotalQtyCol).setNote(noteText);
...
// "Qty Sold" and the revenue columns come directly from Bills + BillItems,
// NOT from StockLog - this matters because a product doesn't have to be
...
//
// --- Revenue is broken into Gross / Discount / Net, same as the Universal
// Report (see computeBillTotals_ / apiGetUniversalReport for the full
// explanation of the two-stage discount): "Gross Revenue" is qty*price
// BEFORE any discount; "Item Discount" and "Bill Discount" are that day's
// two discount types added up separately; "Net Revenue" is what was
// ACTUALLY EARNED that day, after BOTH discounts are taken off Gross -
// this is the figure that should be read as "the real revenue for the
// day", not Gross Revenue. Merges the active spreadsheet with every
// registered archive.
function apiGetDailyReport() {
  const map = {}; // "date||productNameLowercase" -> { date, product, addedToStock, qtySold, grossRevenue, itemDiscount, billDiscount, revenue }

  const getRow = (dateStr, product) => {
    const key = dateStr + '||' + String(product).trim().toLowerCase();
    if (!map[key]) map[key] = { date: dateStr, product: product, addedToStock: 0, qtySold: 0, grossRevenue: 0, itemDiscount: 0, billDiscount: 0, revenue: 0 };
    return map[key];
  };
  ...
  allDbs_().forEach(db => {
    ...
    if (billsSh && itemsSh) {
      ...
      for (let i = 1; i < itemsData.length; i++) {
        ...
        const bill = billMap[String(item[0])];
        if (!bill) continue;
        const dateStr = fmtDate_(bill[1], 'yyyy-MM-dd', db.ss);
        const row = getRow(dateStr, item[1]);

        // Same gross -> item discount -> bill discount -> net breakdown as
        // apiGetUniversalReport() - see the comment there for why
        // billDiscountAmount is derived from the stored (already-net)
        // LineTotal instead of recomputed independently.
        const unitPrice = Number(item[2]) || 0;
        const qty = Number(item[3]) || 0;
        const lineTotal = Number(item[4]) || 0; // net, after BOTH discounts
        const itemDiscountPercent = Number(item[5]) || 0;
        ...
        row.qtySold += qty;
        row.grossRevenue += grossAmount;
        row.itemDiscount += itemDiscountAmount;
        row.billDiscount += billDiscountAmount;
        row.revenue += lineTotal; // net - what was really earned that day
      }
    }
  });
...
const DAILY_REPORT_HEADERS = [
  'Date', 'Service / Treatment', 'Added To Stock', 'Qty Sold',
  'Gross Revenue', 'Item Discount', 'Bill Discount', 'Net Revenue (Actually Earned)'
];
...
function buildDailyReportSheet_() {
  ...
  if (billsSh && itemsSh) {
    ...
    for (let i = 1; i < itemsData.length; i++) {
      ...
      const bill = billMap[String(item[0])];
      if (!bill) continue;
      const dateStr = fmtDate_(bill[1], 'yyyy-MM-dd');
      const row = getRow(dateStr, item[1]);

      // Same gross -> item discount -> bill discount -> net breakdown as
      // buildUniversalReportSheet_() - see the comment there.
      const unitPrice = Number(item[2]) || 0;
      const qty = Number(item[3]) || 0;
      const lineTotal = Number(item[4]) || 0; // net, after BOTH discounts
      const itemDiscountPercent = Number(item[5]) || 0;
...
function buildDashboardSheet() {
  ...
  // KPI numbers
  const kpiLabels = ['Total Sales (Bills)', 'Total Sales (Amount)', 'Total Products/Services Sold', 'Unique Patients'];
